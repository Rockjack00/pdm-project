...
from pdmproject.cspace.tree import SparseOccupancyTree, TopologyNode, BinaryLeafNode
...
class test_tree(unittest.TestCase):
    ...
    def test_init(self):
        with self.assertRaises(AssertionError):
            tree = SparseOccupancyTree(0, self.resolution)

        with self.assertRaises(AssertionError):
            tree = SparseOccupancyTree(self.dimension, 0)

        with self.assertRaises(AssertionError):
            tree = SparseOccupancyTree(1000, self.resolution)

        with self.assertRaises(AssertionError):
            tree = SparseOccupancyTree(self.dimension, 
                                       self.resolution,
                                       limits=[0,1])
    ...
    def test_z_order(self):
        points = np.array([[0b10000, 0b11111, 0b11010],  # 16, 31, 26 / 32
                           [0b01100, 0b10011, 0b00111]]) # 12, 19, 7  / 32
        z_orders = np.array([0b010110010110111,
                             0b110110101001010])
        self.assertTrue(np.all(self.tree.z_order(points) == z_orders))

    def test_locate(self):
        self.tree.limits = np.array([[-1, -1, -1],
                                     [31, 31, 31]])
        points = np.array([[15.43, 29.75, 25],          # 0b10000,0b11111,0b11010
                           [   11,    18,  6]])         # 0b01100,0b10011,0b00111
        voxels = np.array([0b010110010110111,
                           0b110110101001010])
    ...
    def test_get_configurations(self):
        self.tree.limits = np.array([[-1, -1, -1],
                                     [31, 31, 31]])
        points = np.array([[15, 30, 25],          # 0b10000,0b11111,0b11010
                           [11, 18,  6]])         # 0b01100,0b10011,0b00111
        z_orders = [0b010110010110111,
                    0b110110101001010]
        # voxel level
        lower = points
        upper = lower + 1
        ...
        # depth = 2
        lower_shallow = np.array([[15, 23, 23],          # 0b10000,0b11000,0b11000
                                   [7, 15, -1]])         # 0b01000,0b10000,0b00000
        upper_shallow = lower_shallow + 8
        limits0 = self.tree.get_configurations(z_orders[0], 2)
        self.assertTrue(np.all(limits0 == np.vstack((lower_shallow[0,:],
                                                     upper_shallow[0,:]))))
        limits1 = self.tree.get_configurations(z_orders[1], 2)
        self.assertTrue(np.all(limits1 == np.vstack((lower_shallow[1,:],
                                                     upper_shallow[1,:]))))
    ...
    def test__insert(self):
        ...
        self.assertEqual(parent.children[index], out)
        self.assertEqual(out.d, self.dimension)
    ...
    def test__traverse(self):
        voxels = [0b010110010110111, # 010 110 010 110 111
                  0b110110101001010, # 110 110 101 001 010
                  0b000111101001010] # 000 111 101 001 010
        ...
        self.assertEqual(root.children, self.tree._root.children)
        ...
        self.assertEqual(self.tree._root.children[0b111], children[0])
        ...
        self.assertEqual(self.tree._root.children[0b010], children[0])
    ...
    def test_is_full(self):
        voxels = [0b010110010110111, # 010 110 010 110 111
                  0b110110101001010, # 110 110 101 001 010
                  0b000111101100010] # 000 111 101 001 010
        ...
        is_set, node_stack = self.tree.is_full(voxels[0], 
                                               depth=3, 
                                               node_stack=[])
        ...
        is_set, node_stack_out = self.tree.is_full(voxels[1], 
                                                   depth=5, 
                                                   node_stack=[])
        self.assertTrue(is_set)
        self.assertEqual(node_stack_out, [(0b010, children[0]),
                                      (0b001, children[1]),
                                      (0b101, None)])
    ...
    def test_get_neighbors(self):
        ...
        voxels = [0b000000000000000, # 000 000 000 000 000
                  0b111000000000000] # 111 000 000 000 000
        directions = [0b111111, # all neighbors
                      0b000001, # +x
                      0b000100, # +z
                      0b001000, # -x
                      0b100010] # -z, +y
        ...
        self.assertEqual(neighbors, [(0b101010000000000, 0b000010), # +y
                                     (0b011000000000000, 0b100000)])# -z
        ...
        self.assertEqual(neighbors, [(0b000000000000001, 0b000001), # +x
                                     (0b000000000000010, 0b000010), # +y
                                     (0b000000000000100, 0b000100)])# +z
        ...
        self.assertEqual(neighbors, [(0b000000001000000, 0b000001), # +x
                                     (0b000000010000000, 0b000010), # +y
                                     (0b000000100000000, 0b000100)])# +z

    def test__assemble_voxel_key(self):
        voxels = [0b010110010110111, # 010 110 010 110 111
                  0b110110101001010, # 110 110 101 001 010
                  0b000000101001010] # 000 000 101 001 010

        # assemble voxel key for full stack
        node_stack = [(0b111, TopologyNode(self.dimension)),
        ...
        node_stack = [(0b010, TopologyNode(self.dimension)),
    ...
    def test_get_smallest_neighbors(self):
        ...
        voxels = [0b000000000000000, # 000 000 000 000 000
                  ...
                  0b000000000000100] # 000 000 000 000 100
        directions = [0b111111, # all neighbors
                      0b001000, # -x
                      0b110100] # -z, -y, +z 
        ...
        self.assertEqual(set(neighbors), {(0b000000000000110, 1),
                                          (0b000000000000101, 1),
                                          (0b000000000000011, 1)})
        neighbors = self.tree.get_smallest_neighbors(voxels[0], directions[0], depth=4)
        self.assertEqual(set(neighbors), {(0b000001000000000, 4),
                                          (0b000010000000000, 4),
                                          (0b000100000000000, 4)})
        ...
        self.assertEqual(set(neighbors), {(0b000000000000111, 5),
                                          (0b010000000000111, 5),
                                          (0b001000000000111, 5),
                                          (0b011000000000111, 5),
                                          (0b000010000000111, 4),
                                          ...
                                          (0b000000000000001, 1)}) # -y neighbor, no -z neighbors
        ...
        self.assertEqual(set(neighbors), {(0b000000000000101, 1), # +x
                                          (0b000000000000110, 1), # +y
                                          (0b000000000100000, 2), 
                                          (0b000000000101000, 2),
                                          (0b000000000110000, 2),
                                          (0b000000000111000, 2)}) # ^^^ -z neighbors


    def test_flood_fill(self):
        seeds = np.array([[0b00000, 0b00000, 0b00000],       #  0,  0,  0
                          [0b10000, 0b11111, 0b11010],       # 16, 31, 26 / 32
                          [0b01100, 0b10011, 0b00111]]) / 32 # 12, 19,  7 / 32
